...
dash.register_page(__name__, name='Season Records - Postseason')
...
season = pd.read_csv('s3://isfl-surrender-bot/AllStats/pseason.csv')
...
layout = dbc.Container([
    ...
    dcc.Markdown('''
        # ISFL Postseason Season Records

        Choose a statistic:
    ''', className='pt-2'),
    html.Div(dcc.Dropdown(id='dropdown0_sp', options=dpdown0_s, value='Passing'),className="dbc w-50"),
    html.Div(dcc.Dropdown(id='dropdown_sp', options=dpdown_s, value='Pass Yds'),className='dbc w-50'),
    html.Div(id='table-container_sp',  className='tableDiv w-90'),
], className='dbc')

@callback(
    Output('dropdown_sp','options'),
    Input('dropdown0_sp','value')
)
...
@callback(
    Output('table-container_sp','children'),
    Input('dropdown_sp', 'value')
)
