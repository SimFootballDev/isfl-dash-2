...
dash.register_page(__name__, name='Career Records - Postseason')
...
pcareer = pd.read_csv('s3://isfl-surrender-bot/AllStats/pcareer.csv')
...
layout = dbc.Container([
    ...
    dcc.Markdown('''
        # ISFL Postseason Career Records

        Choose a statistic:
    ''', className='pt-2'),
    html.Div(dcc.Dropdown(id='dropdown0_p', options=dpdown0, value='Passing'),className="dbc w-50"),
    html.Div(dcc.Dropdown(id='dropdown_p', options=dpdown, value='Pass Yds'),className='dbc w-50'),
    html.Div(id='table-container_p',  className='tableDiv w-90'),
],className='dbc')

@callback(
    Output('dropdown_p','options'),
    Input('dropdown0_p','value')
)

def changeStats_p(value):
    if value == 'Passing':
...
@callback(
    Output('table-container_p','children'),
    Input('dropdown_p', 'value')
)

def display_table_p(value):
    ...
    df_temp = pcareer.loc[pcareer[limit[0]] >= limit[1],['FullName', value]].rename(columns={'FullName':'Player'})
