...
dash.register_page(__name__, name='Franchise Season Records')
...
teamDictS = {}
for t in teamList:
    teamDictS[t] = pd.read_csv('s3://isfl-surrender-bot/AllStats/Teams/%s_season.csv'%t)
...
layout = dbc.Container([
    ...
    dcc.Markdown('''
        ...
        Choose a team:
    ''', className='pt-2'),
    html.Div(dcc.Dropdown(id='tdropdown0t_s', options=teamdrop, value='AUS'),className="dbc w-50"),
    dcc.Markdown("Choose a statistic:",className='pt-2'),
    html.Div(dcc.Dropdown(id='tdropdown0_s', options=dpdown0, value='Passing'),className="dbc w-50"),
    html.Div(dcc.Dropdown(id='tdropdown_s', options=dpdown, value='Pass Yds'),className='dbc w-50'),
    html.Div(id='ttable-container_s',  className='tableDiv w-90'),
],className='dbc')

@callback(
    Output('tdropdown_s','options'),
    Input('tdropdown0_s','value')
)
...
@callback(
    Output('ttable-container_s','children'),
    Input('tdropdown0t_s', 'value'),
    Input('tdropdown_s', 'value'))
