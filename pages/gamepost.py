...
dash.register_page(__name__, name='Game Records - Postseason')
...
game = pd.read_csv('s3://isfl-surrender-bot/AllStats/pgame.csv',low_memory=False)
...
layout = dbc.Container([
    ...
    dcc.Markdown('''
        # ISFL Postseason Game Records

        Choose a statistic:
    ''', className='pt-2'),
    html.Div(dcc.Dropdown(id='dropdown0_gp', options=dpdown0_s, value='Passing'),className="dbc w-50"),
    html.Div(dcc.Dropdown(id='dropdown_gp', options=dpdown_s, value='Pass Yds'),className='dbc w-50'),
    html.Div(id='table-container_gp',  className='tableDiv w-90'),
],className='dbc')

@callback(
    Output('dropdown_gp','options'),
    Input('dropdown0_gp','value')
)
...
@callback(
    Output('table-container_gp','children'),
    Input('dropdown_gp', 'value')
)
