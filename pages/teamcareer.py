...
dash.register_page(__name__, name='Franchise Career Records')
...
teamDictSP = {}
for t in teamList:
    teamDict[t] = pd.read_csv('s3://isfl-surrender-bot/AllStats/Teams/%s.csv'%t)
...
layout = dbc.Container([
    ...
    dcc.Markdown('''
        ...
        Choose a team:
    ''', className='pt-2'),
    html.Div(dcc.Dropdown(id='tdropdown0t', options=teamdrop, value='AUS'),className="dbc w-50"),
    dcc.Markdown("Choose a statistic:",className='pt-2'),
    html.Div(dcc.Dropdown(id='tdropdown0', options=dpdown0, value='Passing'),className="dbc w-50"),
    html.Div(dcc.Dropdown(id='tdropdown', options=dpdown, value='Pass Yds'),className='dbc w-50'),
    html.Div(id='ttable-container',  className='tableDiv w-90'),
],className='dbc')

@callback(
    Output('tdropdown','options'),
    Input('tdropdown0','value')
)
...
@callback(
    Output('ttable-container','children'),
    Input('tdropdown0t', 'value'),
    Input('tdropdown', 'value'))
