...
dash.register_page(__name__, name='Franchise Career Records - Postseason')
...
# career = pd.read_csv('s3://isfl-surrender-bot/AllStats/pcareer.csv')
teamList = ['AUS', 'AZ', 'BAL', 'BER', 'CHI', 'COL', 'CTC', 'HON', 'NOLA', 'NYS', 'OCO', 'SAR', 'SJS', 'YKW']

# teamList = list(np.sort(game.Team.unique()))
teamDictP = {}
for t in teamList:
    teamDictP[t] = pd.read_csv('s3://isfl-surrender-bot/AllStats/Teams/%s_p.csv'%t)
...
layout = dbc.Container([
    ...
    dcc.Markdown('''
        # Franchise Postseason Career Records

        Choose a team:
    ''', className='pt-2'),
    html.Div(dcc.Dropdown(id='tdropdown0t_p', options=teamdrop, value='AUS'),className="dbc w-50"),
    dcc.Markdown("Choose a statistic:",className='pt-2'),
    html.Div(dcc.Dropdown(id='tdropdown0_p', options=dpdown0, value='Passing'),className="dbc w-50"),
    html.Div(dcc.Dropdown(id='tdropdown_p', options=dpdown, value='Pass Yds'),className='dbc w-50'),
    html.Div(id='ttable-container_p',  className='tableDiv w-90'),
],className='dbc')

@callback(
    Output('tdropdown_p','options'),
    Input('tdropdown0_p','value')
)
...
@callback(
    Output('ttable-container_p','children'),
    Input('tdropdown0t_p', 'value'),
    Input('tdropdown_p', 'value'))
